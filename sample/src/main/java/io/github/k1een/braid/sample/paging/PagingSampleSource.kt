...
internal class PagingSampleSource(private val loadDelayMillis: Long = DEFAULT_LOAD_DELAY_MILLIS) :
    PagingSource<Int, PagingSampleItem>() {

    private val didEmitDemoFailure = AtomicBoolean(false)
    private val items = createItems()

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, PagingSampleItem> {
    ...
    override fun getRefreshKey(state: PagingState<Int, PagingSampleItem>): Int? = state.anchorPosition
    ...
    private fun createItems(): List<PagingSampleItem> = List(TOTAL_ITEM_COUNT) { index ->
