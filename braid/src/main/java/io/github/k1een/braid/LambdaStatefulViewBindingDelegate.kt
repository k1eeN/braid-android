...
// Constructor parameters directly capture the explicit stateful DSL lifecycle callbacks.
...
internal class LambdaStatefulViewBindingDelegate<
    ...
    override fun bind(binding: VB, state: State, item: Item, payloads: List<Any>) {
    ...
    override fun failedToRecycle(binding: VB, state: State): Boolean = failedToRecycleCallback.invoke(binding, state)
}
